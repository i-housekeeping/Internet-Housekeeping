...
function SoundManager(smURL,smID) {
  ...
  this._createMovie = function(smID,smURL) {
    ...
    var htmlEmbed = '<embed name="'+smID+'" id="'+smID+'" src="'+smURL+'" width="100%" height="100%" quality="high" allowScriptAccess="always" quality="high" '+(_s.useHighPerformance && !_s.useMovieStar?'wmode="transparent" ':'')+'bgcolor="'+_s.bgColor+'" pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash"></embed>';
  ...
  this.handleFocus = function() {
    ...
    setTimeout(_s.waitForExternalInterface,1000);
    // detach event
  ...
  this.play = function(oOptions) {
    ...
    if (_t.paused) {
      ...
      if (_t._iO.onplay) _t._iO.onplay.apply(_t);
  ...
  this.stop = function(bAll) {
    if (_t.playState == 1) {
      ...
      if (_t._iO.onstop) _t._iO.onstop.apply(_t);
  ...
  this.pause = function() {
    if (_t.paused || _t.playState == 0) return false;
  ...
  this._onmetadata = function(oMetaData) {
    ...
    _s.wD('SMSound.onmetadata() complete');
  };
