...
class User < ActiveRecord::Base
  ...
  #validates_presence_of     :login, :email, :domain
  ...
  def self.authenticate(login, password)
    #u = find_by_login(login) # need to get the password_salt
    u = find :first , :conditions => ["login = ? and activated_at IS NOT NULL AND record_sts='ACTV'", login]
    u && u.authenticated?(password) ? u : nil
  end
  
  # Encrypts some data with the password_salt.
  def self.encrypt(password, password_salt)
    Digest::SHA1.hexdigest("--#{password_salt}--#{password}--")
  end
  
  # Encrypts the password with the user password_salt
