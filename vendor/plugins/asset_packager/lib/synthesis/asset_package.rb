      ...
      def merged_file
        ...
        @sources.each {|s| 
          File.open("#{@asset_path}/#{s}.#{@extension}", "r") { |f| 
             content = f.read
             merged_file += fix_css_urls(content, s) + "\n" 
          }
        }
        merged_file
      end
    
      def fix_css_urls(source, url)         
        
        if @asset_type == "stylesheets"   
          subst = ""
          log "I am here with url : #{url} " 
           
          unless url.index("/css/").nil?
              subst = url[0..url.index("/css/")] 
              source.gsub!("url(../","url(#{subst}")
              log "Index of css #{url.index("/css/")} and subst #{subst} "
          end
          unless url.index("/modules/").nil? 
              subst = url[0..url.rindex("/")] 
              source.gsub!("url(images","url(#{subst}/images")
              log "Index of module #{url.index("/modules/")} and subst #{subst}"
          end 
          unless source.index("/wallpapers/").nil?
            source.gsub!("../resources/wallpapers/","../javascripts/housekeeping/resources/wallpapers/")
          end
